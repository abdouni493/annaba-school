...
interface DataActions {
  ...
  /**
   * EFFACE LE SOLDE ACTUEL DE TOUT UN EMPLOI DU TEMPS.
   *
   * C'est le pendant, pour UN SEUL emploi, du script de purge livré dans
   * `supabase/purge-2026-08-27-soldes-eleves.sql` : après elle, chacun de ses
   * élèves redevient redevable de la TOTALITÉ de sa scolarité sur cet emploi —
   * plus un dinar versé, plus une séance payée d'avance, plus un crédit.
   *
   * Ce qui part :
   *  1. les versements rattachés à cet emploi (`subscriptionId`), quelle que
   *     soit leur provenance ;
   *  2. leur reflet en caisse — l'entrée du guichet, et la sortie jumelle
   *     quand c'est l'école qui avait avancé l'argent ;
   *  3. les compteurs d'argent des inscriptions : solde à zéro, et « séances
   *     payées » ramené sur « séances consommées » ;
   *  4. les frais nés d'une avance de l'école SUR CET EMPLOI — le versement
   *     qu'ils remboursaient n'existe plus, les réclamer ferait payer deux fois ;
   *  5. les scolarités portées sur le salaire d'un père et PAS ENCORE retenues :
   *     l'enfant les redoit lui-même. Celles déjà retenues restent, cet
   *     argent-là est réellement sorti.
   *
   * Ce qui reste : LES PRÉSENCES. L'élève est bien venu au cours, et effacer
   * son pointage effacerait aussi le travail de l'enseignant.
   *
   * Irréversible.
   */
  clearSubscriptionSolds: (
    ...
    ok: boolean;
    /** élèves dont le solde a été remis à plat */
    students?: number;
    /** versements effacés */
    payments?: number;
    /** total encaissé qui vient d'être annulé */
    amount?: number;
  }>;
  /**
...
export const useData = create<DataStore>((set, get) => ({
  ...
  clearSubscriptionSolds: async (subscriptionId) => {
    const db = get();
    const sub = db.subscriptions.find((s) => s.id === subscriptionId);
    if (!sub) return { ok: false };

    // 1. LES VERSEMENTS DE CET EMPLOI DU TEMPS.
    //
    //    `subscriptionId` est ce qui fait d'un versement une SCOLARITÉ : c'est
    //    l'emploi du temps qu'il crédite. Un règlement de frais porte un
    //    `chargeId` et aucun emploi — il reste où il est.
    const wiped = db.payments.filter((p) => p.subscriptionId === subscriptionId);
    const amount = wiped.reduce((t, p) => t + Math.max(0, Math.round(p.amountPaid || 0)), 0);
    const enrollments = db.enrollments.filter((e) => e.subscriptionId === subscriptionId);
    // Rien versé et aucune cagnotte à plat à remettre : ne rien écrire du tout,
    // sinon chaque clic ferait repartir toute la table vers la base pour rien.
    const alreadyFlat = enrollments.every(
      (e) => !e.balance && e.paidSeances === e.consumedSeances,
    );
    if (wiped.length === 0 && alreadyFlat) return { ok: true, students: 0, payments: 0, amount: 0 };
    const wipedIds = new Set(wiped.map((p) => p.id));

    // 2. LEUR REFLET EN CAISSE.
    //
    //    Aucune colonne ne relie une ligne de caisse à son versement : c'est
    //    l'horodatage AU MILLIÈME, le type et le montant qui les apparient,
    //    exactement comme quand on supprime un encaissement à l'unité. Chaque
    //    ligne n'est consommée qu'une fois, donc deux versements identiques
    //    n'emportent pas deux fois la même écriture. Les entrées qui ne sont
    //    PAS des soldes de scolarité — une séance libre, le règlement d'un
    //    frais — sont écartées d'office.
    const seanceCash = new Set<string>();
    for (const g of db.groupSeances) {
      if (g.cashInId) seanceCash.add(g.cashInId);
      if (g.cashOutId) seanceCash.add(g.cashOutId);
    }
    const isSoldCash = (c: CashTransaction) =>
      !seanceCash.has(c.id) &&
      !(c.description ?? "").startsWith("Séance libre") &&
      !(c.description ?? "").startsWith("Frais «");

    const cashIds = new Set<string>();
    const takeCash = (type: CashTransaction["type"], date: string, value: number) => {
      const row = db.cash.find(
        (c) =>
          !cashIds.has(c.id) &&
          c.type === type &&
    ...
    for (const p of wiped) {
      // Une scolarité prise sur le salaire du père n'a jamais traversé le
      // tiroir : il n'y a rien à retirer de la caisse.
      if (p.paidFrom === "teacher_salary" || p.paidFrom === "teacher_debt") continue;
      const credit = Math.max(0, Math.round(p.amountPaid || 0));
      takeCash("student_payment", p.date, credit);
      // L'école avait avancé l'argent : l'entrée portée au crédit de l'élève
      // et la sortie qui l'a financée partent ensemble.
      if (p.paidFrom === "school_cash") takeCash("student_debt", p.date, -credit);
    }

    // 3. LES FRAIS NÉS D'UNE AVANCE DE L'ÉCOLE SUR CET EMPLOI.
    //
    //    Un frais `school_advance` dit « la famille doit à l'école la
    //    scolarité que la caisse a réglée à sa place ». Cette scolarité vient
    //    d'être remise à devoir : réclamer les deux ferait payer deux fois.
    const droppedCharges = new Set(
      db.studentCharges
        .filter(
          (c) =>
            c.origin === "school_advance" &&
            (c.subscriptionId === subscriptionId ||
              (c.sourcePaymentId ? wipedIds.has(c.sourcePaymentId) : false)),
        )
        .map((c) => c.id),
    );

    // 4. LES SCOLARITÉS PORTÉES SUR UN SALAIRE, PAS ENCORE RETENUES.
    //
    //    Elles attendaient la prochaine paie du père. Le versement qu'elles
    //    remboursaient n'existe plus, donc l'enfant redoit sa scolarité
    //    lui-même. Celles DÉJÀ retenues restent : cet argent est sorti.
    const droppedDebts = new Set(
      db.teacherChildDebts
        .filter((d) => d.subscriptionId === subscriptionId && !d.paid)
        .map((d) => d.id),
    );

    set((state) => ({
      payments: state.payments.filter((p) => !wipedIds.has(p.id)),
      cash: state.cash.filter((c) => !cashIds.has(c.id)),
      // 5. LES COMPTEURS D'ARGENT DES INSCRIPTIONS.
      //
      //    `balance` est ce qui reste dans la cagnotte de l'élève sur cet
      //    emploi. Sans versement elle est vide — et « séances payées »
      //    retombe donc sur « séances consommées » : plus rien n'est réglé
      //    d'avance, mais les séances déjà suivies restent comptées.
      enrollments: state.enrollments.map((e) =>
        e.subscriptionId === subscriptionId
          ? { ...e, balance: 0, paidSeances: e.consumedSeances }
          : e,
      ),
      studentCharges: state.studentCharges.filter((c) => !droppedCharges.has(c.id)),
      teacherChildDebts: state.teacherChildDebts.filter((d) => !droppedDebts.has(d.id)),
    }));

    return {
      ok: true,
      students: new Set([...enrollments.map((e) => e.studentId), ...wiped.map((p) => p.studentId)])
        .size,
      payments: wiped.length,
      amount,
    };
  },
